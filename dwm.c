...
bstack(Monitor *m) {
	...
	for (i = 0, mx = m->gap->gappx, tx = m->wx + m->gap->gappx , c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
		...
		//	mx += WIDTH(c);
			...
			resize(c, tx, ty + m->gap->gappx, w - (2 * c->bw), h - (2 * c->bw) - m->gap->gappx, 0);
			//if (tw != m->ww)
			//	tx += WIDTH(c);
